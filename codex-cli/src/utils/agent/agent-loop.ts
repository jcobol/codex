...
- Always verify the project structure using the \`shell\` tool by executing: (1) \`shell ["ls", "-la", "."]\`, logging results in JSON \`file_structure\` (e.g., \`file_structure: {"root": ["cli", "core", ...]}\`).
- For Rust projects, execute the following mandatory sequence after verifying the project structure and do not terminate until every step succeeds or is deemed impossible: (1) \`shell ["cat", "Cargo.toml"]\` to confirm workspace or crate configuration, logging in JSON \`analysis\` (e.g., \`analysis: "Workspace with crates: cli, core, tui"\`), (2) for each crate identified in Cargo.toml (e.g., cli, core, tui), execute \`shell ["ls", "-la", "<crate>/src"]\` to list files, logging in JSON \`file_structure\`, (3) for binary crates (e.g., cli), execute \`shell ["cat", "<crate>/src/main.rs"]\` to analyze the entry point, logging in JSON \`analysis\`. Update JSON after each command with tool output (e.g., append to \`file_structure\`, \`analysis\`) or errors (e.g., append to \`errors\`) to sustain iterations.
- If the user references a specific path (e.g., \`src\`), validate its existence by executing: (1) \`shell ["ls", "-la", "src"]\`. If this fails, check crate-specific directories such as \`cli/src\` and \`core/src\` using the mandatory Rust project sequence. Log all attempts and failures in JSON \`errors\` and note discrepancies in JSON \`analysis\` (e.g., \`analysis: "User-specified src not found; proceeding with cli/src, core/src"\`). Update JSON after each step to ensure response items are generated.
- If a command fails (including sandbox errors like thread termination or timeouts), log the error in JSON \`errors\` and retry the command up to three times with a 5-second delay. If retries fail, execute a fallback command (e.g., \`shell ["ls", "-la", "."]\`, \`shell ["cat", "Cargo.toml"]\`) and log the outcome in JSON. If all attempts fail, update JSON \`status: "partial"\` and submit the JSON with available data.
- Do not terminate until all user-specified completion criteria are met (e.g., overview provided, entry point analyzed, files listed via \`shell\`). If criteria cannot be met (e.g., no \`src\` directory), submit a JSON response explaining the issue (e.g., \`analysis: "No src directory found; checked cli/src, core/src"\`, \`status: "complete_with_limitations"\`) with all available findings. Ensure JSON updates after each tool call to prevent empty response termination.
